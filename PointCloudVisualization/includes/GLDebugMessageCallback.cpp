...
#ifdef _DEBUG
    // If the program breaks here, it means that OpenGL has encountered an
    // error. Tips for tracing it:
    //  1. Read the console logs.
    //  2. By inspecting the Call Stack, look for the OpenGL operation that
    //     triggered the error. (the arrow should point to the line right after
    //     the one that triggered it)
    //  3. You may continue the program's execution (green triangle - Continue / F5)
    //     ... or terminate it (red triangle - Stop / Shift + F5)
    __debugbreak();
#endif
}